...
namespace CityFlow.Sim.Tests
{
    public class SimStatsTests
    {
        ...
        static SimStats StatsForWithGrid(CityGrid g, in SimConfig cfg)
        {
            var dm = new DemandMap(cfg);
            var net = new RoadNetwork(g);
            dm.Reassign(g, net);
        ...
        [Test]
        public void JamPresent_StabilityBelowOne()
        {
            var cfg = Cfg(15f);
            cfg.StabilityJamWeight = 1f;
            float globalOnly = StatsFor(StraightCity(), cfg).Stability01;
            float localized = StatsForWithGrid(StraightCity(), cfg).Stability01;

            Assert.Less(localized, 1f);
            Assert.Less(localized, globalOnly);
        }
    }
}
