...
겹치면 **스폰 실패 → 다음 틱 재시도**. **새 우선권·새 예외 0개.**

> **정정 2026-08-03 (리뷰 P1 지적 → 감독 실코드 확인).**
> 최초 작성본은 "신호가 빨강이면 Entry에서 대기"라고 썼는데 **현 상태기계와 다르다.**
> `RoadQueueNetwork.cs:789-815`는 Entry/Conflict를 signalGate 확인 **전에** 내부 전진시키고,
> 신호 검사는 접근 차량의 다음 타일에서만 일어난다(`L866`). 기존 계약도
> `RoadQueueDeviceTests.cs:158-171,360-381`에서 **"교차로 내부 차량은 적색이어도 비운다"** 이다.
> 차고 진출은 신호 통제 대상이 아니므로 **출발 스폰에 신호 게이트를 걸지 않는다.**
> 이는 기존 계약과 일치하며 새 예외를 만들지 않는다. `TryEnqueueAtIntersection`에
> `signalGate`/tick 입력을 추가하지 않는 이유이기도 하다.
